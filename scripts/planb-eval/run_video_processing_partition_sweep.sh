#!/usr/bin/env bash
# Sweep independently compressed Private-WS partitions and IAA in-flight jobs
# for the frozen video-processing-python snapshot.
set -euo pipefail

root=${ROOT:-/home/ubuntu/snapshare-fourfn-eval}
stamp=${STAMP:-20260820-video-processing-iaa-partition-sweep-r1}
repetitions=${REPETITIONS:-10}
runner=$root/run_fourfn_planb_ab.sh
parser=$root/summarize_planb_partition_sweep.py
...
if [[ ${PREFLIGHT_ONLY:-0} == 1 ]]; then
  echo "preflight passed: active_policies=$active_policy_count frequency_khz=4000000 smt_active=$smt_active iaa_work_queues=$queue_count mongo_ready=$mongo_ready"
  exit 0
fi
...
outer_log=$result_root/launcher.log
{
  ...
  echo "start=$(date -Is)"
  echo "host=$(hostname)"
  echo "workload=video-processing-python"
...
for setting in "${sweep_settings[@]}"; do
  ...
  WORKLOAD_LIST=video-processing-python \
...
python3 "$parser" "$result_root"
{
  ...
  echo "mongo_ready_after=$({ timeout 3 bash -c '</dev/tcp/10.0.0.11/27017' 2>/dev/null && echo 1; } || echo 0)"
} >>"$result_root/provenance.txt"
echo "VIDEO_PROCESSING_PARTITION_SWEEP_COMPLETE result_root=$result_root"
