#!/usr/bin/env python3
"""Aggregate the five-workload fixed-CPU IAA-only 8-WQ matrix."""
...
def main() -> None:
    ...
    settings = metadata.get("iaa_settings", "1:1 2:2 4:4 8:8").split()
    configs = {
        ("iaa_deflate", int(setting.split(":", 1)[0]), int(setting.split(":", 1)[1]))
    ...
    expected_children = 2 * len(configs)
    if len(children) != expected_children:
        raise SystemExit(f"expected {expected_children} completed children, found {len(children)}")
    ...
    print(
        f"FIXED4G_IAA_WQ8_SUMMARY calls={len(all_calls)} variants={len(all_variants)} "
